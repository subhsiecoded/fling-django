...
def register(request):
    if request.method == "POST":
        form = UserRegistrationForm(request.POST)  # Instantiate the form with POST data
        if form.is_valid():
            user = form.save(commit=False)
            user.set_password(form.cleaned_data['password1'])
            user.save()
            login(request, user)
            return redirect('fling_list')  # Redirect to fling_list after successful registration
    else:
